...
- **Real database queries** against Pakistani and Daraz e-commerce data with PKR pricing
...
- **Analytics dashboard** powered by the Olist Brazilian E-Commerce Dataset
...
Download all datasets from Kaggle and place them as shown below.

**Olist dataset** — goes directly in `data/`:
```
...
**Pakistani / Daraz datasets** — raw files go in `data/raw/`, cleaned outputs must be present in `data/cleaned/`:
```
data/raw/pakistan_ecommerce.csv      (from ranasagheerkhan/pakistans-largest-e-commerce-dataset)
data/raw/daraz_products.csv          (from muhammadmurtazasani/daraz-top-sales-products-dataset-2024)
data/raw/daraz_discounts.csv         (from neloybarman018/daraz-11-11-top-selling-product-data)
...
The cleaned files are generated by the data preprocessing pipeline. If they are not present, run the cleaning script before starting the app.
...
The project uses two separate dataset sources serving different purposes.

### Pakistani / Daraz Datasets — AI Assistant Database

Three Kaggle datasets were downloaded, cleaned, and merged into the SQLite `database/support.db` that the AI assistant queries in real time:

| Dataset | Source | Used for |
|---|---|---|
| Pakistan's Largest E-Commerce Dataset | [ranasagheerkhan on Kaggle](https://www.kaggle.com/datasets/ranasagheerkhan/pakistans-largest-e-commerce-dataset) | Orders, order status, payment methods (`orders.csv`, `payments.csv`) |
| Daraz Top Sales Products Dataset 2024 | [muhammadmurtazasani on Kaggle](https://www.kaggle.com/datasets/muhammadmurtazasani/daraz-top-sales-products-dataset-2024) | Product catalog with PKR prices and star ratings (`products.csv`, `inventory.csv`) |
| Daraz 11.11 Top Selling Product Data | [neloybarman018 on Kaggle](https://www.kaggle.com/datasets/neloybarman018/daraz-11-11-top-selling-product-data) | Discounted products and sale prices (`discounts.csv`) |

Raw files are stored in `data/raw/`. After cleaning and merging they produce five files in `data/cleaned/`:

```
data/
├── raw/
│   ├── pakistan_ecommerce.csv   # Orders and transaction records
│   ├── daraz_products.csv       # Product listings with PKR pricing
│   └── daraz_discounts.csv      # Discounted / sale products
└── cleaned/
    ├── products.csv             # product_id, product_name, category, price_pkr, rating, review_count
    ├── inventory.csv            # product_id, stock_quantity, availability
    ├── orders.csv               # order_id, order_status, product_name, order_total, payment_method, customer_id
    ├── payments.csv             # order_id, payment_method, amount
    └── discounts.csv            # product_name, original_price, discounted_price, discount_percent
```

This data is what powers all AI assistant features — product search, pricing queries, order tracking, cancellations, refunds, discounts, and inventory checks. Prices are in PKR throughout.

### Olist Brazilian E-Commerce Dataset — Analytics Dashboard

The [Olist Dataset](https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce) (100,000+ orders, 2016–2018) is used exclusively to power the Executive Dashboard and Support Insights tab. Its rich relational structure — linking orders, reviews, products, and sellers — makes it well suited for generating operational KPIs such as delivery delay rates, monthly order trends, and problematic product category analysis.
...
The Olist data is loaded in-memory via `data_loader.py` and never written to `support.db` — it is only used for dashboard visualisations and insight calculations.
